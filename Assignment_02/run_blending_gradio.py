...
def create_mask_from_points(points, img_h, img_w):
    ...
    if len(points) >= 3:
        ...
        draw.polygon([tuple(point) for point in points.tolist()], fill=255)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    lap_kernel = lap_kernel.repeat(foreground_img.shape[1], 1, 1, 1)

    foreground_laplacian = F.conv2d(foreground_img, lap_kernel, padding=1, groups=foreground_img.shape[1])
    blended_laplacian = F.conv2d(blended_img, lap_kernel, padding=1, groups=blended_img.shape[1])

    foreground_mask_expanded = foreground_mask.expand(-1, foreground_img.shape[1], -1, -1) > 0.5
    background_mask_expanded = background_mask.expand(-1, blended_img.shape[1], -1, -1) > 0.5

    foreground_values = foreground_laplacian[foreground_mask_expanded]
    blended_values = blended_laplacian[background_mask_expanded]

    if foreground_values.numel() > 0 and blended_values.numel() > 0:
        valid_count = min(foreground_values.numel(), blended_values.numel())
        loss = F.mse_loss(blended_values[:valid_count], foreground_values[:valid_count])
