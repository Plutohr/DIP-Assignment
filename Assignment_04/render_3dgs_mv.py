...
def require_cuda_device(device_arg):
    """Return a CUDA device or fail loudly instead of falling back to CPU."""
    if not device_arg.startswith('cuda'):
        raise ValueError('Rendering is configured for CUDA. Use --device cuda.')
    if not torch.cuda.is_available():
        raise RuntimeError(
            'CUDA is not available to PyTorch. Install CUDA-enabled PyTorch and '
            'run from a WSL session that can access the RTX 3050.'
        )
...
def parse_args():
    ...
    p.add_argument('--maximum_points', type=int, default=3000,
                   help='Must match the point count used during training; <=0 keeps all points')
    return p.parse_args()
