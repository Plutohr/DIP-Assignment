...
def read_images_text(path):
    ...
    sorted_images = dict(sorted(images.items(), key=lambda x: natural_key(x[1]['name'])))
...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num=3000):
        ...
        # Sample 3D points to keep the dense tensor renderer practical on 4-6GB GPUs.
        if maximum_pts_num is not None and maximum_pts_num > 0 and len(self.points3D_xyz) > maximum_pts_num:
            generator = torch.Generator().manual_seed(0)
            indices = torch.randperm(len(self.points3D_xyz), generator=generator)[:maximum_pts_num]
            self.points3D_xyz = self.points3D_xyz[indices]
            self.points3D_rgb = self.points3D_rgb[indices]
            print(f"Sampled {maximum_pts_num} / {len(points3D)} COLMAP points for 3DGS initialization")
