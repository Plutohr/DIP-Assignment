...
def require_cuda_device(device_arg: str) -> torch.device:
    """Return a CUDA device or fail loudly instead of falling back to CPU."""
    if not device_arg.startswith("cuda"):
        raise ValueError("Task 2 training is configured to run on CUDA only. Use --device cuda.")
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is not available to PyTorch, so training would run on CPU. "
            "Install CUDA-enabled PyTorch and run from a WSL session that can access the RTX 3050."
        )
...
def parse_args():
    ...
    parser.add_argument('--downsample_factor', type=int, default=8,
                      help='Image downsample factor used during training')
    parser.add_argument('--maximum_points', type=int, default=3000,
                      help='Maximum COLMAP points used to initialize gaussians; <=0 keeps all points')
...
def main():
    ...
    # Initialize dataset
    maximum_points = None if args.maximum_points <= 0 else args.maximum_points
    dataset = ColmapDataset(
        args.colmap_dir,
        downsample_factor=args.downsample_factor,
        maximum_pts_num=maximum_points,
    )
