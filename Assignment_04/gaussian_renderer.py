...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        mahalanobis = torch.einsum('nhwi,nij,nhwj->nhw', dx, inv_covs, dx)
        norm = 1.0 / (2.0 * np.pi * torch.sqrt(det))
        gaussian = norm.view(N, 1, 1) * torch.exp(-0.5 * mahalanobis)
    ...
    def forward(
        ...
        alphas = (opacities.view(N, 1, 1) * gaussian_values).clamp(0.0, 0.99)  # (N, H, W)
